...
//		BooleanExpression expression = qBoard.writer.contains("둘리");
		...
		BooleanExpression expression = qBoard.title.contains("안녕");
		...
		// contains함수를 사용하여 SQL에 LIKE 연산자가 추가됨
		// 함께 LIKE 키워드 앞뒤에 %가 추가됨
		// 이때 %를 문자 그대로 처리하기 위해 ESCAPE 이스케이프 문자를 사용함
	...
	void 다중항목검색테스트2() {
		...
		// 첫번째 조건: 게시물 번호가 2보다 큰 게시물	 // 비교 gt > 2
		BooleanExpression expression = qBoard.no.gt(2);

		// 두번째 조건: 내용에 '안녕하세요'이 포함되는 게시물
		BooleanExpression expression2 = qBoard.content
											.contains("안녕하세요");
